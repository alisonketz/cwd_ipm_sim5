...
modelcode <- nimbleCode({
  ...
  ### Age effects - Fawns
  for (k in 1:nknots_age_fawn) {
    ln_b_age_survival_fawn[k] ~ dnorm(0, tau_age_survival_fawn)
    b_age_survival_fawn[k] <- exp(ln_b_age_survival_fawn[k])
  }
  tau_age_survival_fawn ~ dgamma(1, 1)

  for (t in 1:51) {
    age_effect_survival_temp_fawn[t] <- inprod(b_age_survival_fawn[1:nknots_age_fawn],
                                     Z_age_fawn[t, 1:nknots_age_fawn])
  }
  mu_age_effect_survival_temp_fawn <- mean(age_effect_survival_temp_fawn[1:51])
  
  for (t in 1:51) {
    age_effect_survival[t] <-  age_effect_survival_temp_fawn[t] -
                               mu_age_effect_survival_temp_fawn
  }
  ...
  for (t in 1:(nT_age-51)) {
    age_effect_survival_temp[t] <- inprod(b_age_survival[1:nknots_age],
                                     Z_age[t, 1:nknots_age])
  }
  mu_age_effect_survival_temp <- mean(age_effect_survival_temp[1:(nT_age-51)])
  
  for (t in 52:nT_age) {
    age_effect_survival[t] <-  age_effect_survival_temp[t-51] -
