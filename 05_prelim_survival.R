...
# quant_age <- .2
# knots_age <- c(1, round(quantile(dat$df_fit$r_age,
...
### fawns - decreasing convex basis expansion
knots_age_fawn <- round(seq(1,51,length=6))
delta_i <- decconvex(1:51, knots_age_fawn)
delta <- t(delta_i$sigma - delta_i$center.vector)
Z_age_fawn <- delta / max(delta)
nknots_age_fawn <- dim(Z_age_fawn)[2]

### adults - decreasing convex basis expansion
knots_age <- round(seq(52,nT_age,length=6))
delta_i <- convex(52:nT_age, knots_age, pred.new = FALSE)
delta <- t(rbind(delta_i$sigma - 
                t(delta_i$x.mat %*%
                delta_i$center.vector)))
delta <- delta / max(delta)
